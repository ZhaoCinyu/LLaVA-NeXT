...
sys.path.append(os.path.join(current_dir, 'eval', 'self_extend_patch'))

import llava.model.self_extend_patch as SE
import pdb

def modify_class_of_instance(instance, target_class_name, new_class, visited_instances=None):
    pass
...
def apply(loaded_model, group_size, window_size, enable_flash_attention=False, scale_base=-1, flash_attention_impl="triton",
    ...
    if 'Llama' in arch_name:
        if enable_flash_attention:
            if mixture:
                # pdb.set_trace()
                self_extend_attention_forward = partial(SE.Llama.flash_mixture_self_extend_forward,
            ...
            elif flash_attention_impl == "flash_attn" or flash_attention_impl == "triton":
        ...
        if enable_flash_attention:
            if mixture:
                self_extend_attention_forward = partial(SE.Qwen2.flash_mixture_self_extend_forward,
                                                group_size_1=group_size, 
                                                group_size_2=window_size,
                                                scale_base=scale_base)
                ...
                if (not modifed_1) or (not modifed_2):
                    raise Exception(f"Failed to modify the attention method of {arch_name}")
                print("Using qwen2 flash self_extend!!")
            else:
