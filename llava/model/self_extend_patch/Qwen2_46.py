...
def flash_mixture_self_extend_forward(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    ...
    **kwargs,
    ...
    if q_len == 1:
        ...
        # normal attn
        # cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)
        normal_query_states, normal_key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids)
        ...
        _value_states = value_states.transpose(1, 2)
        ...
        from transformers.modeling_flash_attention_utils import _flash_attention_forward as ori_flash_attention_forward
        normal_attn_output = ori_flash_attention_forward(
            normal_query_states,
            normal_key_states,
            _value_states,
            attention_mask,
    ...
    if normal_attn_output is not None:
        ## random mixture
        # normal_attn_mask = torch.randint(0, 2, (bsz, 1, self.num_heads, 1), dtype=torch.bool, device=normal_attn_output.device)
        
        ## fixed mixture
        normal_attn_mask = torch.zeros(bsz, 1, self.num_heads, 1, dtype=torch.bool, device=normal_attn_output.device)
        # normal_attn_mask[:, :, :self.num_heads//2, :] = True # 14 heads
        # normal_attn_mask[:, :, :self.num_heads//4, :] = True # 7 heads
        # normal_attn_mask[:, :, :1, :] = True # 1 heads
        # normal_attn_mask[:, :, :2, :] = True # 2 heads
        # normal_attn_mask[:, :, :3, :] = True # 3 heads
        # normal_attn_mask[:, :, :4, :] = True # 4 heads
        normal_attn_mask[:, :, :12, :] = True # 6 heads

        ## local-aware mixture
        # normal_attn_mask = torch.zeros(bsz, 1, self.num_heads, 1, dtype=torch.bool, device=normal_attn_output.device)
        # normal_attn_mask[:, :, head_indices, :] = True

        attn_output = torch.where(normal_attn_mask, normal_attn_output, attn_output)
...
def flash_self_extend_forward_extend(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    ...
    **kwargs,
    ...
    ## fixed selection
    # last_heads = query_states[:, -extend_num:, :, :]
    
    ## random selection 
    ## (random each forward) last_heads_index = torch.randperm(self.num_heads, device=query_states.device)[:extend_num]
    # last_heads = query_states[:, self.last_heads_index, :, :]
    
    # query_states = torch.cat([query_states, last_heads], dim=1) 
    ...
    if q_len == 1:
        ...
        ## ori code starts
        decode_key_states = repeat_kv(decode_key_states, self.num_key_value_groups)
        decode_value_states = repeat_kv(value_states, self.num_key_value_groups)
        ## ori code ends

        ## fixed selection starts
        # last_key_states = decode_key_states[:, -1:, :, :].expand(-1, extend_num, -1, -1)
        # last_value_states = value_states[:, -1:, :, :].expand(-1, extend_num, -1, -1)
        # decode_key_states = repeat_kv(decode_key_states, self.num_key_value_groups)
        # decode_value_states = repeat_kv(value_states, self.num_key_value_groups)
        # decode_key_states = torch.cat([decode_key_states, last_key_states], dim=1)
        # decode_value_states = torch.cat([decode_value_states, last_value_states], dim=1)
        ## fixed selection ends

        ## random selection starts
        # import pdb; pdb.set_trace()
        # kv_index = self.last_heads_index//self.num_key_value_groups
        # kv_index = kv_index.view(1, extend_num, 1, 1).expand(1, extend_num, decode_key_states.shape[-2], self.head_dim).to(torch.int64)
        # last_key_states = torch.gather(decode_key_states, dim=1, index=kv_index)
        # last_value_states = torch.gather(value_states, dim=1, index=kv_index)
        ...
        # decode_key_states = torch.cat([decode_key_states, last_key_states], dim=1)
        # decode_value_states = torch.cat([decode_value_states, last_value_states], dim=1)
        ## random selection ends

        decode_key_states = decode_key_states.transpose(1, 2).contiguous()
        decode_value_states = decode_value_states.transpose(1, 2).contiguous()

        # import pdb; pdb.set_trace()
        attn_output = flash_attn_func(decode_query_states,#1, 1, 28, 128
                                      decode_key_states,
                                      decode_value_states,
                                      attn_dropout, 
                                      softmax_scale=None, 
        ...
        # random selection starts
        # kv_index = self.last_heads_index//self.num_key_value_groups
        # kv_index = kv_index.view(1, extend_num, 1, 1).expand(1, extend_num, group_key_states.shape[-2], self.head_dim).to(torch.int64)
        # last_group_key_states = torch.gather(group_key_states, dim=1, index=kv_index)
        # last_neighbor_key_states = torch.gather(neighbor_key_states, dim=1, index=kv_index)
        # last_value_states = torch.gather(value_states, dim=1, index=kv_index)
        # random selection ends

        ## fixed selection starts
        # last_neighbor_key_states = neighbor_key_states[:, -1:, :, :].expand(-1, extend_num, -1, -1)
        # last_group_key_states = group_key_states[:, -1:, :, :].expand(-1, extend_num, -1, -1)
        # last_value_states = value_states[:, -1:, :, :].expand(-1, extend_num, -1, -1)
        ## fixed selection ends
        ...
        # neighbor_key_states = torch.cat([neighbor_key_states, last_neighbor_key_states], dim=1)
        # group_key_states = torch.cat([group_key_states, last_group_key_states], dim=1)
        # value_states = torch.cat([value_states, last_value_states], dim=1)
    ...
    attn_output = attn_output.reshape(bsz, q_len, self.head_dim * (self.num_heads+extend_num)).contiguous()

    ## fixed selection
    original_weight = self.o_proj.weight.data  # (hidden_size, num_heads * head_dim)
    last_heads_weight = original_weight[:, -extend_num * self.head_dim:]
    o_proj = torch.cat([original_weight, last_heads_weight], dim=1)
    
    ## random selection
    # import pdb; pdb.set_trace()
    # original_weight = self.o_proj.weight.data.reshape(self.hidden_size, self.num_heads, self.head_dim)  # (hidden_size, num_heads * head_dim)
    # last_weight = original_weight[:,self.last_heads_index,:]
    # o_proj = torch.cat([original_weight, last_weight], dim=1).view(self.hidden_size,-1)
